@php
    ...
    $customFaviconUrl = config('statamic.cp.custom_favicon_url');
@endphp
...
<title>{{ config('statamic.cp.custom_cms_name', config('app.name', 'Dric Interior')) }}</title>

@if ($customFaviconUrl)
    <link rel="icon" type="image/png" href="{{ $customFaviconUrl }}" sizes="32x32">
    <link rel="icon" type="image/png" href="{{ $customFaviconUrl }}" sizes="16x16">
...
@if (config('statamic.cp.custom_css_url'))
